...
def generate_namespaced_nav2_params(base_params_path, namespace):
    ...
    # FIX: Applica la patch ricorsiva per aggiungere i namespace
    patch_recursively(namespaced_params, namespace)
    
    # FIX: Ripristina forzatamente 'map' per i frame globali dopo la patch.
    # Questo è necessario perché la logica precedente potrebbe non gestire
    # correttamente tutte le strutture annidate (es. costmap).
    def fix_map_frames(data):
        if isinstance(data, dict):
            for key, value in data.items():
                if key in frame_keys and isinstance(value, str):
                    if value == f"{namespace}/map":
                        data[key] = 'map'
                fix_map_frames(value)
        elif isinstance(data, list):
            for item in data:
                fix_map_frames(item)

    # Applica la correzione alla struttura finale dei parametri
    fix_map_frames(namespaced_params)
